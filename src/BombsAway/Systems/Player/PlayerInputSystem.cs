    ...
    public class PlayerInputSystem : IUpdateSystem, IFixedUpdateSystem
    {
        ...
        public void FixedUpdate(Context context)
        {
            foreach (Entity entity in context.Entities)
            {
                ...
                if (moved)
                {
                    ...
                    BombsAwaySoundPlayer.Instance.SetGlobalParameter(LibraryServices.GetLibrary().Roll, _cachedInputAxis.X);
                    BombsAwaySoundPlayer.Instance.SetGlobalParameter(LibraryServices.GetLibrary().Thrust, -_cachedInputAxis.Y);
                }
                else
                {
        ...
        public void Update(Context context)
        {
                ...
                && context.World.GetEntitiesWith(typeof(DoNotPauseComponent)).Count() == 0) 
            {
                ...
                BombsAwaySoundPlayer.Instance.PlayEvent(LibraryServices.GetLibrary().UiNavigate, Murder.Core.Sounds.SoundProperties.None);
            }
        }
